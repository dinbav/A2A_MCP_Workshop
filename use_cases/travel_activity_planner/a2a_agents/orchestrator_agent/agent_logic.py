...
class OrchestratorAgentLogic:
    ...
    async def _select_agents(self, user_input: str) -> list[RemoteAgent]:
        """Return agents scored above 0, keeping those within 50 % of the best score."""
        ...
        if scored:
            ...
            return [a for s, a in scored if s >= threshold]

        return list(self._remote_agents.values())[:1]
    ...
    async def _send_to_agent(self, agent: RemoteAgent, user_input: str,
        ...
        try:
            ...
            async for event in agent.client.send_message(msg):
    ...
    async def stream(self, user_input: str, context_id: str = None) -> AsyncGenerator[dict, None]:
        ...
        try:
            agents = await self._select_agents(user_input)
